...
void ewk::VtoElectronTreeFiller::SetBranches()
{
  ...
  SetBranch( &V_mt,        "mt");
  SetBranch( &V_mt_corrected,        "mt_corrected");
  SetBranch( &V_mtMVA,        "mtMVA");
  SetBranch( &V_mtMVA_corrected,        "mtMVA_corrected");
  SetBranch( &V_px,        "px");
  SetBranch( &V_py,        "py");
  SetBranch( &V_pz,        "pz");
  SetBranch( &V_E,         "e");
...
void ewk::VtoElectronTreeFiller::fill(const edm::Event& iEvent, int vecBosonIndex)
{
 ...
 /////// Pileup density "rho" in the event from fastJet pileup calculation /////
  ...
  double deltaphi = fabs(Vboson->daughter(0)->phi() - Vboson->daughter(1)->phi());
  if (deltaphi > 6.283185308) deltaphi -= 6.283185308;
  if (deltaphi > 3.141592654) deltaphi = 6.283185308 - deltaphi;

  V_mt_corrected = sqrt(2.0*Vboson->daughter(0)->pt()*Vboson->daughter(1)->pt()*(1.0-cos(deltaphi)));

  //  std::cout<<" dphi1 "<<Vboson->daughter(0)->phi()-Vboson->daughter(1)->phi()<<" dphi 2 "<<deltaphi<<" mt1 "<<V_mt<<" mt2 "<<V_mt_corrected<<std::endl;

  if (metMVA->size() > 0){
        V_mtMVA = sqrt(2.0*Vboson->daughter(0)->pt()*(*metMVA)[0].et()*
              (1.0-cos(Vboson->daughter(0)->phi()-(*metMVA)[0].phi())));

        deltaphi = fabs( Vboson->daughter(0)->phi()-(*metMVA)[0].phi() ) ;
	if (deltaphi > 6.283185308) deltaphi -= 6.283185308;
	if (deltaphi > 3.141592654) deltaphi = 6.283185308 - deltaphi;

        V_mtMVA_corrected = sqrt(2.0*Vboson->daughter(0)->pt()*(*metMVA)[0].et()*
                            (1.0-cos(deltaphi)));
  }
