...
const TString inDataDir  = "/gwteray/users/gerosa/MergedNtuples_v1/MuonChannelExoLeptonID_Analysis/";
...
void kanamuon::myana(double myflag, bool isQCD, int runflag)
{
   ...
   if ( !isQCD ) {
      ...
      if (myflag == 20121002 || myflag == -200){
         ...
         Loop( h_events, h_events_weighted, 20121002,runflag, outDataDir + "RD_mu_STopS_Tbar_CMSSW532");
      }
      ...
      if (myflag == 20126030 || myflag == -400){
        InitCounters( inDataDir + "mu_X_WW_lvjj_PS600p-narrow-JHU.root", h_events, h_events_weighted);
        myChain = new TChain("WJet");
        myChain->Add(                    inDataDir + "mu_X_WW_lvjj_PS600p-narrow-JHU.root");
        SetBtagEfficiencyMap(std::string(BtagDir+"Bulk_CMSSW532.root"));
        Init(myChain);Loop( h_events, h_events_weighted, 20126030,runflag, outDataDir + "RD_mu_X_WW_lvjj_PS600p-narrow-JHU");
      }


      if (myflag == 20126031 || myflag == -400){
        InitCounters( inDataDir + "mu_X_WW_lvjj_SM600p-narrow-JHU.root", h_events, h_events_weighted);
        myChain = new TChain("WJet");
        myChain->Add(                    inDataDir + "mu_X_WW_lvjj_SM600p-narrow-JHU.root");
        SetBtagEfficiencyMap(std::string(BtagDir+"Bulk_CMSSW532.root"));
        Init(myChain);Loop( h_events, h_events_weighted, 20126031,runflag, outDataDir + "RD_mu_X_WW_lvjj_SM600p-narrow-JHU");
      }

      if (myflag == 20126032 || myflag == -400){
        InitCounters( inDataDir + "mu_X_WW_lvjj_SM600m-narrow-JHU.root", h_events, h_events_weighted);
        myChain = new TChain("WJet");
        myChain->Add(                    inDataDir + "mu_X_WW_lvjj_SM600m-narrow-JHU.root");
         SetBtagEfficiencyMap(std::string(BtagDir+"Bulk_CMSSW532.root"));
        Init(myChain);Loop( h_events, h_events_weighted, 20126032,runflag, outDataDir + "RD_mu_X_WW_lvjj_SM600m-narrow-JHU");
      }

      if (myflag == 20126033 || myflag == -400){
        InitCounters( inDataDir + "mu_X_WW_lvjj_PS600m-narrow-JHU.root", h_events, h_events_weighted);
        myChain = new TChain("WJet");
        myChain->Add(                    inDataDir + "mu_X_WW_lvjj_PS600m-narrow-JHU.root");
         SetBtagEfficiencyMap(std::string(BtagDir+"Bulk_CMSSW532.root"));
        Init(myChain);Loop( h_events, h_events_weighted, 20126033,runflag, outDataDir + "RD_mu_X_WW_lvjj_PS600m-narrow-JHU");
      }

      if (myflag == 20126034 || myflag == -400){
        InitCounters( inDataDir + "mu_X_WW_lvjj_SM1000m-narrow-JHU.root", h_events, h_events_weighted);
        myChain = new TChain("WJet");
        myChain->Add(                    inDataDir + "mu_X_WW_lvjj_SM1000m-narrow-JHU.root");
         SetBtagEfficiencyMap(std::string(BtagDir+"Bulk_CMSSW532.root"));
        Init(myChain);Loop( h_events, h_events_weighted, 20126034,runflag, outDataDir + "RD_mu_X_WW_lvjj_SM1000m-narrow-JHU");
      }

      if (myflag == 20126035 || myflag == -400){
        InitCounters( inDataDir + "mu_X_WW_lvjj_PS1000m-narrow-JHU.root", h_events, h_events_weighted);
        myChain = new TChain("WJet");
        myChain->Add(                    inDataDir + "mu_X_WW_lvjj_PS1000m-narrow-JHU.root");
         SetBtagEfficiencyMap(std::string(BtagDir+"Bulk_CMSSW532.root"));
        Init(myChain);Loop( h_events, h_events_weighted, 20126035,runflag, outDataDir + "RD_mu_X_WW_lvjj_PS1000m-narrow-JHU");
      }

      if (myflag == 20126036 || myflag == -400){
        InitCounters( inDataDir + "mu_X_WW_lvjj_SM2000m-narrow-JHU.root", h_events, h_events_weighted);
        myChain = new TChain("WJet");
        myChain->Add(                    inDataDir + "mu_X_WW_lvjj_SM2000m-narrow-JHU.root");
         SetBtagEfficiencyMap(std::string(BtagDir+"Bulk_CMSSW532.root"));
        Init(myChain);Loop( h_events, h_events_weighted, 20126036,runflag, outDataDir + "RD_mu_X_WW_lvjj_SM2000m-narrow-JHU");
      }

      if (myflag == 20126037 || myflag == -400){
        InitCounters( inDataDir + "mu_X_WW_lvjj_PS2000m-narrow-JHU.root", h_events, h_events_weighted);
        myChain = new TChain("WJet");
        myChain->Add(                    inDataDir + "mu_X_WW_lvjj_PS2000m-narrow-JHU.root");
         SetBtagEfficiencyMap(std::string(BtagDir+"Bulk_CMSSW532.root"));
        Init(myChain);Loop( h_events, h_events_weighted, 20126037,runflag, outDataDir + "RD_mu_X_WW_lvjj_PS2000m-narrow-JHU");
      }
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	for( int iJet = 0; iJet < (bvetoedM.size()+btaggedM.size()) ; iJet++){
	  ...
	  double SF_down    = jetSF.at(iJet) - jetSF_e_down.at(iJet);
          ...
          if( SF_up == 0 || SF_central == 0 || SF_down == 0 ){ SF_central = 1 ; SF_up = 1 ; SF_down = 1;} 
          if( jetEff.at(iJet) == 0  ) continue ;
