...
int main (int argc, char *argv){
  ...
  std::string TableOutputName  = "PowhegResult_VBFSelection.txt";
  ...
  for( unsigned int iFile = 0; iFile <NameOfInputFile.size(); iFile ++){
	...
	if(f2V1pt >= f2V2pt){ LeadingPt=f2V1pt;   TrailingPt=f2V2pt; LeadingEta=f2V1eta; TrailingEta=f2V2eta; 
     ...
     if(itMapLeadingJet->first  < TagJetPtCut  ) continue;
     if(itMapTrailingJet->first < TagJetPtCut  ) continue;
     ...
     if( Mjj < MjjTagJetCutMin ) continue ;

     // Jet Veto Selection 
     bool doCentralJetVeto = false ;
     float TrailingJetEta = itMapTrailingJet->second.first;

     itMapTrailingJet++;

     for( ;itMapTrailingJet != SortedJetCleaned->rend(); itMapTrailingJet++){

       if(itMapTrailingJet->first > TagJetPtCut && ((itMapTrailingJet->second.first < TrailingJetEta && 
	  itMapTrailingJet->second.first > itMapLeadingJet->second.first) || 
	  (itMapTrailingJet->second.first > TrailingJetEta && itMapTrailingJet->second.first < itMapLeadingJet->second.first)) ) doCentralJetVeto = true;
     }

     if(doCentralJetVeto==true) continue ;
