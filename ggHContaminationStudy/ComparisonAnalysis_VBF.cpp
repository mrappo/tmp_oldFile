...
// Complie with g++ -Wall `root-config --libs --glibs --cflags` -o ComparisonAnalysis_VBF ComparisonAnalysis_VBF.cpp
...
int main (int argc, char** argv){
  ...
  std::string VirtualFileDirectory = "VBFAnalysis_JOB/VBF_Virtual/" ;
  std::string RealFileDirectory    = "VBFAnalysis_JOB/VBF_Real/" ;
  std::string ListOfVirtualFile    = "VBFHWW_VirtualList.txt" ;
  std::string ListOfRealFile       = "VBFHWW_RealList.txt" ;
  ...
  command = "ls "+VirtualFileDirectory+" | grep virt | grep .dat  | sed -e s%wbf%%g  | sed -e s%dat%%g | tr \"_\" \" \" "+
            " | awk '{print \"echo @wbf_\"$6\"_\"$7\"_\"$8\"_\"$9\"_\"$10\" \"$7\" \"$8\" \"$9\" \"$10\" @ >> "+ListOfVirtualFile+" \"}'"+
            " | tr \"@\" \"\\\"\" | /bin/sh" ;
  ...
  command = "ls "+RealFileDirectory+" | grep real | grep .dat  | sed -e s%wbf%%g  | sed -e s%dat%%g | tr \"_\" \" \" "+
            " | awk '{print \"echo @wbf_\"$6\"_\"$7\"_\"$8\"_\"$9\"_\"$10\" \"$7\" \"$8\" \"$9\" \"$10\" @ >> "+ListOfRealFile+" \"}'"+
            " | tr \"@\" \"\\\"\" | /bin/sh" ;
