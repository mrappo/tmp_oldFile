...
void StatDialogMVAEffs::PrintResults( const MethodInfo* info )
{
  ...
  ofstream mfile;
  ofstream mfile_2;
  mfile.open ("MATTEO_max_significance_file.txt", ios::out | ios::app);
  mfile << "\nBin\t\t";
  mfile << maxbin ;
  mfile << "\nmax Significance\t";
  mfile << info->sSig->GetXaxis()->GetBinCenter( maxbin );
  mfile.close();
  mfile_2.open("MATTEO_bin_file.txt");
  mfile_2 << "<Bin ibin=\""<< maxbin <<"\"";
  mfile_2.close();
}

void MATTEO_mvaeff( TString fin = "TMVA.root", 
