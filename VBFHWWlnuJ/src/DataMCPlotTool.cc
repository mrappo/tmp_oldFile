...
void LatexCMS (const double & lumi, const std::string & LeptonType, const bool & isLabel){
  ...
  if(LeptonType == "muon" || LeptonType == "mu") latex.DrawLatex(0.4,0.962,Form("CMS preliminary, %.1f fb^{-1} at #sqrt{s} = 8 TeV, mu+jets",(float)lumi/1000));
  else if(LeptonType == "electron" || LeptonType == "el") latex.DrawLatex(0.4,0.962,Form("CMS preliminary, %.1f fb^{-1} at #sqrt{s} = 8 TeV, el+jets",(float)lumi/1000));
